...
const SelectConversation: FC<SelectConversationProps> = ({
  ...
  if (conversation.users.length === 2)
    return (
      <Link
        to={`/${conversationId}`}
        className={`hover:bg-dark-lighten relative my-3 flex items-stretch gap-2 rounded-md bg-white py-2 px-5 drop-shadow-md transition duration-300`}
      >
        <img
  ...
  return (
    <Link
      to={`/${conversationId}`}
      className={`hover:bg-dark-lighten group relative my-3 flex items-stretch gap-2 rounded-md bg-white py-2 px-5 drop-shadow-md transition duration-300`}
    >
